...
class SnpFilter:
    ...
    def __init__(
        ...
        genotype_quality=20,
        raw_read_depth=5,
        quality_read_depth=5,
        depth_in_reference=5,
        depth_in_variant=5,
    ):
        """
    ...
    def _write_snp_records(self, record):
        """ Writes snp records. """
    ...
    def check_threshold_value(self, recordvalue, testvalue):
        """ Test snp record value to threshold value.
		args:
    ...
    def check_genotype_quality(self, record, sample, testvalue):
        ...
        return self.check_threshold_value(
            self.get_genotype_quality(record, sample), testvalue
        )

    def check_quality_read_depth(self, record, sample, testvalue):
        """ function to call read depth quality of alt base in the snp cal.l"""
        return self.check_threshold_value(
            self.get_quality_read_depth(record, sample), testvalue
        )

    def check_raw_read_depth(self, record, sample, testvalue):
        """ function to check raw read depth of alt base in the snp call."""
        return self.check_threshold_value(
            self.get_raw_read_depth(record, sample), testvalue
        )
    ...
    def filter(self):
        ...
        for record in self.vcf_reader:
            if self.passed_filter(record, samplename) == True:
                self._write_snp_records(record)
